...
from utils import (
    ...
    OUTPUT_CSV, CALIBRATION_FILE, TILT_CORRECTION_ANGLE, PersonSelector, MultiviewTriangulator
)
...
# Pipeline Settings
CONFIDENCE_THR = 0.4 # Minimum confidence to trust a 2D point
ROBUST_TRIANGULATION = True # Keep this True to fix multi-view errors
...
def main():
    ...
    print(INFO + f"Initializing HRNet-w48 (RAW) on {device}...")
    ...
    # --- PROCESSING LOOP ---
    print(INFO + "Starting Raw Processing...")
    ...
    while True:
        ...
        for j in range(23):
            ...
            for cam_idx in range(len(caps)):
                if cam_idx not in current_indices: continue
        ...
        if frame_idx % 2 == 0:
            ...
            if len(valid) > 0:
                ax.scatter(valid[:,0], valid[:,2], -valid[:,1], c="red")
                ax.set_xlim(-2, 2); ax.set_ylim(-2, 2); ax.set_zlim(0, 3)
                ax.set_title(f"Raw Tracking: Frame {frame_idx}")
            plt.pause(0.001)
