...
def main():
    ...
    try:
        while True:
            ...
            # visualization
            # resize and show deeply.
            display_h = 360  # smaller size = faster processing
            previews = []
            ...
            cv2.imshow("Multi-View Capture", combined)

            # crucial timing control
            # instead of a fixed waitKey(1), we wait appropriately to match 25 FPS
            # 1000ms / 25fps = 40ms per frame.

            key = cv2.waitKey(1) & 0xFF  # keep this short to poll inputs fast
